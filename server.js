...
const mongoose = require('mongoose'); // 修正：確保 mongoose 模組被引入
...
// 中間件設定
// 修正：僅保留一個 cors 宣告，並根據你的需求進行配置
app.use(cors({
    origin: '*', // 在開發環境中使用 *，生產環境應改為你的前端網址
    methods: ["GET", "POST"]
}));
...
// 初始化 Socket.IO
const initializeSocketIO = require('./socketHandlers'); // 修正：將引入放在這裡，避免在 `io` 實例創建後才引入
